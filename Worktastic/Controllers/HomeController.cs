...
namespace Worktastic.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            List<JobPosting> jobPosting = new List<JobPosting>();

            if (string.IsNullOrWhiteSpace(query))
            {
                jobPosting = _context.JobPostings.ToList();
            }
            else
            {
                jobPosting = _context.JobPostings
                    .Where(x => x.JobTitle.ToLower().Contains(query.ToLower()))
                    .ToList();
            }

            return PartialView("_JobPostingListPartial", jobPosting);
        }
    }
}
